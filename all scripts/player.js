...
volumeIndicator.style.width = "0"; // Изначально установленная ширина индикатора (0%)
volumeIndicator.style.height = "100%";
volumeIndicator.style.background = "rgba(255, 255, 255, 0.5)";
volumeIndicator.style.borderRadius = "10px 0px 0px 10px";
...
// console.log("click supported:", "ontouchend" in document);
// console.log("touchstart supported:", "ontouchstart" in document);


// Добавляем обработчик события изменения значения ползунка громкости
volumeSeekBar.addEventListener("input", function(event) {
  const volumeValue = event.target.value;
  updateVolume(volumeValue); // Функция для обновления громкости
  updateVolumeIndicatorWidth(volumeValue); // Функция для обновления ширины индикатора громкости
});

// Функция для обновления громкости
function updateVolume(volumeValue) {
  // Применяем новое значение громкости к текущему треку или к общему аудио
  const currentTrack = tracks.find(track => track.playing);
  if (currentTrack) {
    currentTrack.audio.volume = volumeValue / 100;
  } else {
    // Обновляем громкость общего аудио
    audio.volume = volumeValue / 100;
  }
}

// Функция для обновления ширины индикатора громкости
function updateVolumeIndicatorWidth(volumeValue) {
  volumeIndicator.style.width = volumeValue + "%";
}
